...
# 装饰器带参数，其实外加一层函数outer_arg，让装饰器的参数bar执行，定义函数装饰器本身outer且带原函数名foo来作参数，再返回装饰器本身outer地址，
# outer = outer_arg('foo_arg')
# 当装饰器outer执行时，定义了inner函数带原函数foo参数*args,**kwargs，inner函数里可执行原始函数foo, 执行原始函数foo前后都可以自定义一些逻辑。
# 再返回inner函数的地址。
# inner = outer(foo)
# 此时原始函数名foo，已经更换成inner地址，执行inner函数
# inner() 

# 疑问
# 为什么执行原函数foo的逻辑喜欢写在 inner 里面而不是外层的 outer 呢, 是不是两者都可以
# 为什么 foo 明明换了inner 地址，而 inner 里面的却能执行原始的函数foo呢
