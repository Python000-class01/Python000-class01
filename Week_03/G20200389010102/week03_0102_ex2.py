...
def option_parse():
    ...
    parser.add_argument('-n', '--num_process', dest='num_process', action='store', type=int, default=4,
                        help='number of process used for checking')
    parser.add_argument('-f', '--mode', dest='mode', action='store', type=str,
                         default='ping',help='use ping for ip check, tcp for port check')
    parser.add_argument('-ip', '--ip', dest='ip_range', type=str,
                        help='ip range to check, e.g 192.168.0.1-192.168.0.100')
    parser.add_argument('-w', '--write', dest='output', action='store', type=str,
                         default='result.json',help='output file path')
    return parser


#判断单个ip是否符合ip地址的规则
def _ip_odd_check(ip):
    try:
        socket.inet_aton(ip)
        return True
    except socket.error:
...
#判断一个ip段的第一个ip地址和最后一个ip地址是否符合ip地址规则
def _ip_range_check(ip_range):
    ips = ip_range.split('-')
    if len(ips) == 2:
        frist_ip,last_ip = ips
        frist_ip_match = _ip_odd_check(frist_ip)
        last_ip_match = _ip_odd_check(last_ip)
        if frist_ip_match and last_ip_match:
            return True
    return False
...
def ping_scan(ip_range,num_process):
    ...
    all_ip = (str(ipaddress.IPv4Address(ip)) for ip in range(int(frist_ip),int(last_ip)))
...
def main():
    ...
    if mode == 'ping':
        if not _ip_range_check(ip_range):
            parser.print_help()
            print('ip_range is Required parameters!!')
            sys.exit(1)
