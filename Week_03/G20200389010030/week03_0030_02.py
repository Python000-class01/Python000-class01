...
def main():
    ...
    parser.add_argument('-n', '--num_threads', dest='num_threads', type=int, help='number of threads used for checking')
    parser.add_argument('-f', '--mode', dest='mode', type=str, help='use ping for ip check, tcp for port check')
    parser.add_argument('-ip', '--ip', dest='ip_range', type=str, help='ip range to check, e.g 192.168.0.1-192.168.0.100')
    parser.add_argument('-w', '--write', dest='output', type=str, help='output file path')
...
def _ip_range_check(ip_range: str) -> bool:
    ...
    if len(ip_fields) == 2:
        ...
        if start_legal and end_legal:
            range_check = int(ipaddress.IPv4Address(start_ip)) <= int(ipaddress.IPv4Address(end_ip))
            if range_check:
                return True
    return False
...
def tcp_check(ip_str, thread_num):
    ...
    def _tcp_check(ip_port):
        ...
        with mutex:
            result.update({str(ip_port): opened})
...
def ping_check(ip_range, thread_num):
    ...
    def _ping_check(ip):
        res = subprocess.call('ping -n 2 -w 2 {}'.format(ip), stdout=subprocess.PIPE)
        opened = True if res == 0 else False
        print(ip, opened)
        with mutex:
            result.update({str(ip): opened})

    start_ip, end_ip = ip_range.split("-")
    start_ip = ipaddress.IPv4Address(start_ip)
    end_ip = ipaddress.IPv4Address(end_ip)
    ips = (str(ipaddress.IPv4Address(ip_int)) for ip_int in range(int(start_ip), int(end_ip)))
