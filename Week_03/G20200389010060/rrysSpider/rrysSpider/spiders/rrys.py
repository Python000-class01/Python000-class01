...
class RrysSpider(scrapy.Spider):
    ...
    def parse_deep(self,response):
        rank = Selector(response=response).xpath('//p[@class="f4"]/text()').re(
            '\d+')
        print(rank)
        level = Selector(response=response).xpath(
            '//div[@class="level-item"]/img/@src').re('.*\/*([a-e])-big*')
        print(level[0].upper())
        viewcount = Selector(response=response).xpath(
            '//*[@id="score_list"]/div[1]').re('\d+')
        print(viewcount[1])
