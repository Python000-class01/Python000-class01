...
layui.define(['laytpl', 'layer'], function (exports) {
  ...
  Class.prototype.render = function (views, params) {
    var that = this,
      router = layui.router(),
      loadType = 'json';
    if (views.indexOf(GROUP_PATH) != 0 && views.indexOf('http://') != 0 && views.indexOf('https://') != 0) {
    ...
    $.ajax({
      ...
      success: function (res) {
        if (loadType == 'json') {
          if (res.status != 1) {
            return view.error('请求视图文件异常，信息：<br/><span class="red">' + res.info + '</span>');
          }
          var html = '<div>' + res.data.html + '</div>';
        } else {
