...
layui.define('view', function (exports) {
      ...
      theme: function (options) {
          var theme = setter.theme,
            ...
            styleText = laytpl([
              ...
              //选中色
              , '.layui-nav-tree .layui-this,', '.layui-nav-tree .layui-this>a,', '.layui-nav-tree .layui-nav-child dd.layui-this,', '.layui-nav-tree .layui-nav-child dd.layui-this a', '{background-color:{{d.color.selected}} !important;color:{{d.color.textColor}} !important}', '.layui-nav-tree .layui-nav-bar {background-color:{{d.color.selected}} !important;}'
              //logo
  ...
  var events = admin.events = {
    ...
    refresh: function () {
        ...
        var mainframe = $("#mainframe"),
          refreshUrl = mainframe.attr('src');
        $("#iframe-loading").show();
        // console.log('刷新url', refreshUrl);
        mainframe.hide().attr('src', refreshUrl).load(function () {
          $("#iframe-loading").hide();
          mainframe.show();
        });
      }
    ...
    note: function (othis) {
        ...
        events.note.index = admin.popup({
          ...
          content: '<textarea placeholder="内容会存储在本地，这样即便你关掉了浏览器，在下次打开时，依然会读取到上一次的记录"></textarea>',
          resize: false,
    ...
    fullscreen: function (othis) {
      ...
      , //弹出系统消息
    sysMessage: function (othis) {
        var tplPath = othis.data('data_url') || '';
        // console.log(othis.data('data_url'), tplPath);
        if (!tplPath) {
          view.error('请求视图文件异常，信息：<br/><span class="red">url不存在，请配置节点 data-data_url 属性</span>');
          return false;
        }
        admin.popupRight({
          id: 'LAY_adminPopupAbout',
          success: function () {
            $("#LAY-topnav-new-message-tips").hide();
            view(this.id).render(tplPath);
          }
        });
      }
  ...
  ! function () {
    ...
    if (!setter.pageTabs) {
      ...
      $('#LAY_app_tabs').removeClass(HIDE);
      container.removeClass('layadmin-tabspage-none');
    }
