...
def next_page(url):
    ...
    total_page_comments = bs_info.find_all(
        'li', attrs={'class': 'comment-item'})
    comment_num = len(total_page_comments)
    page_list = [url+"hot?p={}".format(i)
                                       for i in range(1, int(total_num/comment_num)+1)]
    return page_list
...
def exec_step():
    page_url_list = (
        next_page(url="https://book.douban.com/subject/5406559/comments/"))
    for page_url in page_url_list:
        comment_list = get_all_comments(page_url)
        sql_val = tuple((str(reviews_snow_nlp(comment)), comment)
