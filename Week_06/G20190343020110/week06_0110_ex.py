...
engine = create_engine(
    "mysql+pymysql://root:hx123456@127.0.0.1:3306/test?charset=utf8", encoding='utf-8',
    echo=True)
...
class DoubanMoiveEntity(Base):
    ...
    comment = Column(String(5000), unique=True)
...
try:
    f = open('douban_movie.csv', 'w', encoding='utf_8', newline='')
    for i in range(1, 20):
...
movies = session.query(DoubanMoiveEntity).order_by(
    DoubanMoiveEntity.score.desc()).limit(20).all()
for movie in movies:
    print(movie.score)
