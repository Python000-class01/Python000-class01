...
class SinaHotspotSpider(CrawlSpider):
    ...
    def parse_sina_news(self, response):
        ...
        try:
            ...
            comment_list = self.get_hot_statistics(response)
            item_loader.add_value("comment_list", comment_list)
    ...
    def get_hot_statistics(self, response):
        ...
        if channel and newsid:
            ...
            if content.get('result') and content.get('result') and content.get('result').get('status').get('code') == 0:
                if content['result'].get('cmntlist'):
                    comment_list = map(lambda x : x.get('content'), content['result'].get('cmntlist') or [])
                    return comment_list
            return []
        else:
            return []
