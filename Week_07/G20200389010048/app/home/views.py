...
def index():
	# 查询数据库的评论数据
	shorts = T1.query.filter().order_by(T1.nc_time2.desc()).all()

	# 选取所有评论的采集时间nc_utime，并按日期统计的评论数
	list_time = []
	for short in shorts:
		list_time.append(short.nc_utime[0:10])
	...
	# 创建饼图数据
	as_count = len(T1.query.filter(T1.nc_sentiment > 0.6).all())
	md_data = []
	md_data_as = {
		'label': '正向评论数',
		'value': as_count
	...
	ns_count = len(T1.query.filter(T1.nc_sentiment < 0.6).all())
	md_data_ns = {
		'label': '负向评论数',
		'value': ns_count
	}
...
def keyword():
	if request.method == 'POST':
		...
		shorts = T1.query.filter(T1.nc_content.like(f'%{keywords}%')).all()
	...
	if request.method == 'POST':
		...
		if startdate != '' and enddate != '':
			if startdate == enddate:
				...
				shorts = T1.query.filter(T1.nc_time2 > start, T1.nc_time2 < end).all()
			...
			start = int(time.mktime(time.strptime(startdate + starttime, "%Y-%m-%d %H:%M:%S")))
			shorts = T1.query.filter(T1.nc_time2 > start).all()
			title = f'大于{startdate + starttime}的评论：'
			return render_template('/home/date.html', shorts=shorts, title=title)
		elif startdate == '' and enddate != '':
			end = int(time.mktime(time.strptime(enddate + endtime, "%Y-%m-%d %H:%M:%S")))
			shorts = T1.query.filter(T1.nc_time2 < end).all()
