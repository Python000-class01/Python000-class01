...
class NewsCommentsService:
    ...
    def get_comments(self, page=0, page_size=25):
        try:
            session = self.db_helper.Session()
            query = session.query(Comments).filter(Comments.news_id == self.news_id)
    ...
    def get_comments_by_date(self, page=0, page_size=25, datestr=None):
        try:
            session = self.db_helper.Session()
            query = session.query(Comments).filter(Comments.news_id == self.news_id).filter(Comments.comment_time == self.__get_date(datestr))
            results = self.db_helper.query(query, page=page, page_size=page_size)
            return [result.to_dict() for result in results]
        except Exception as ex:
            print(ex)
    ...
    def get_number_of_comments_by_date(self, datestr=None):
        try:
            session = self.db_helper.Session()
            result = session.query(Comments.comment_time, func.count('*').label('comments_num')).filter(Comments.news_id == self.news_id).filter(Comments.comment_time == self.__get_date(datestr))
            session.close()
            return {'date': result[0][0].strftime("%Y-%m-%d"), 'count': result[0][1]}
        except Exception as ex:
            print(ex)
            return {'date': self.__get_date(datestr).strftime("%Y-%m-%d"), 'count': 0}

    def get_number_of_comments(self):
        try:
            session = self.db_helper.Session()
            results = session.query(Comments.comment_time, func.count('*').label('comments_num')).filter(Comments.news_id == self.news_id).group_by(Comments.comment_time)
            session.close()
            dates = [result[0].strftime("%Y-%m-%d") for result in results]
            data = [{'date':result[0].strftime("%Y-%m-%d"), 'count': result[1]} for result in results]
            return {'dates': dates, 'data': data}
        except Exception as ex:
            print(ex)
            return {'dates': [], 'data': []}

    def get_number_of_sentiments_by_date(self, datestr=None):
        try:
            session = self.db_helper.Session()
            result = session.query(Comments.comment_time,
                                    func.sum(sql.case([(sql.column('sentiment') >= 0.6, 1)], else_=0)).label(
                                        'positive'),
                                    func.sum(sql.case([(sql.column('sentiment') < 0.6, 1)], else_=0)).label(
                                        'negative')).filter(Comments.news_id == self.news_id).filter(Comments.comment_time == self.__get_date(datestr)).group_by(
                Comments.comment_time)
            session.close()
            return {'date': result[0][0].strftime("%Y-%m-%d"), 'positive': int(result[0][1]), 'negative': int(result[0][2]), 'total': int(result[0][1]) + int(result[0][2])}
        except Exception as ex:
            print(ex)
            return {'date': self.__get_date(datestr).strftime("%Y-%m-%d"), 'count': 0}

    def get_number_of_sentiments(self):
        try:
            session = self.db_helper.Session()
            results = session.query(Comments.comment_time, func.sum(sql.case([(sql.column('sentiment') >= 0.6, 1)], else_=0)).label('positive'),
                          func.sum(sql.case([(sql.column('sentiment') < 0.6, 1)], else_=0)).label('negative')).filter(Comments.news_id == self.news_id).group_by(Comments.comment_time)
            session.close()
            dates = [result[0].strftime("%Y-%m-%d") for result in results]
            data = [{'date':result[0].strftime("%Y-%m-%d"), 'positive': int(result[1]), 'negative': int(result[2]), 'total': int(result[1]) + int(result[2])} for result in results]
            return {'dates': dates, 'data': data}
        except Exception as ex:
            print(ex)
            return {'dates': [], 'data': []}
