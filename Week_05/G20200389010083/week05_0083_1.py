...
keyword = open(path.join(dir, 'sanguo.txt'),encoding='utf-8').read()

tfidf = jieba.analyse.extract_tags(keyword,
topK=50,                   # 权重最大的topK个关键词
withWeight=True)         # 返回每个关键字的权重值
text = ' '.join([w[0] for w in tfidf])
print(text)

background_Image = np.array(Image.open(path.join(dir, 'bg.jpeg')))
img_colors = ImageColorGenerator(background_Image)

wc = WordCloud(
  font_path = path.join(dir, 'msyh.ttf'),
  width = 600,      #默认宽度
  height = 200,     #默认高度
  margin = 2,       #边缘
  ranks_only = None,
