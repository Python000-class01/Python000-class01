...
for keyword in text:
    s = SnowNLP(keyword)
    print(keyword, s.sentiments)

sentiment.train('neg.txt', 'post.txt')
sentiment.save('sentiment.marshal')

import pymysql

db = pymysql.connect('127.0.0.1', 'root', '12345678', 'tube')
...
for keyword in text:
    s = SnowNLP(keyword)
    print(keyword, s.sentiments)
    sql = 'INSERT INTO test(comments,grade) VALUES (\'%s\', "%s")' % (keyword, str(s.sentiments))
    print(sql)
    cursor.execute(sql)
    db.commit()
