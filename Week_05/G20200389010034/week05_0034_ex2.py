...
urls = tuple(
    f'https://movie.douban.com/subject/26752088/comments?start={ page * 20 }&limit=20&sort=new_score&status=P' for page in range(10))  # 把所有url存在元组中
...
def to_sql(df):
    engine = create_engine(
        "mysql+pymysql://{}:{}@{}/{}".format('root', 'root123', 'localhost:3306', 'test'))
    con = engine.connect()
